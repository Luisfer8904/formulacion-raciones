...
// Archivo separado para mejorar el rendimiento del editor
...
function actualizarValores(input) {
  ...
  const peso = (tamanoBachada * porcentaje / 100).toFixed(2);
  const precio = parseFloat(fila.querySelector('input[name^="costo_ingrediente_"]').value) || 0;
  const valorTotal = (precio * peso).toFixed(2);

  fila.querySelector('input[name^="peso_bachada_"]').value = peso;
  fila.querySelector('input[name^="valor_"]').value = valorTotal;
...
function actualizarUnidadSugerido(select, index) {
  ...
  if (unidadInput) {
    ...
    console.log(`Actualizando unidad para índice ${index}: "${unidad}"`);
  }
  if (sugeridoInput) {
    sugeridoInput.value = sugerido;
    console.log(`Actualizando sugerido para índice ${index}: "${sugerido}"`);
  }
...
function aplicarRequerimientosSeleccionados() {
  ...
  // Cargar nutrientes del requerimiento seleccionado con validación
  fetch(`/api/requerimiento/${requerimientoSeleccionado.id}/nutrientes`)
    ...
    .then(data => {
      ...
      // Filtrar solo los nutrientes disponibles
      const nutrientesDisponibles = data.filter(nutriente => nutriente.disponible);
      const nutrientesNoDisponibles = data.filter(nutriente => !nutriente.disponible);
      
      if (nutrientesDisponibles.length === 0) {
        alert('Ninguno de los nutrientes de este requerimiento está disponible en tu lista. Agrega primero los nutrientes necesarios.');
        return;
      }
      
      // Mostrar confirmación si hay nutrientes no disponibles
      if (nutrientesNoDisponibles.length > 0) {
        const mensaje = `⚠️ ATENCIÓN:\n\n` +
          `• ${nutrientesDisponibles.length} nutrientes se aplicarán correctamente\n` +
          `• ${nutrientesNoDisponibles.length} nutrientes NO están disponibles y se omitirán:\n\n` +
          nutrientesNoDisponibles.map(n => `  - ${n.nombre} (${n.mensaje_validacion})`).join('\n') +
          `\n\n¿Deseas continuar aplicando solo los nutrientes disponibles?`;
      ...
      // Agregar solo los nutrientes disponibles
      nutrientesDisponibles.forEach(nutriente => {
        agregarFilaNutriente();
        const ultimaFila = tabla.lastElementChild;
        const select = ultimaFila.querySelector('select');
        const unidadInput = ultimaFila.querySelector(`input[name="unidad_${contadorNutrientes-1}"]`);
        const sugeridoInput = ultimaFila.querySelector(`input[name="sugerido_${contadorNutrientes-1}"]`);
        
        // Seleccionar el nutriente primero
        select.value = nutriente.nutriente_id;
        
        // Actualizar la unidad y sugerido usando la función existente (esto obtiene los datos del select)
        actualizarUnidadSugerido(select, contadorNutrientes-1);
        
        // Después de actualizar con los datos del select, sobrescribir solo el valor sugerido con el del requerimiento
        if (sugeridoInput && nutriente.valor_sugerido) {
          sugeridoInput.value = nutriente.valor_sugerido;
        }
        
        // Los campos min y max se dejan vacíos para que el usuario los llene manualmente
        // NO aplicamos los valores del requerimiento aquí para mantener la funcionalidad manual
      });
      ...
      let mensajeExito = `✅ Requerimientos "${requerimientoSeleccionado.nombre}" aplicados correctamente\n\n`;
      mensajeExito += `• ${nutrientesDisponibles.length} nutrientes cargados exitosamente`;
      
      if (nutrientesNoDisponibles.length > 0) {
        mensajeExito += `\n• ${nutrientesNoDisponibles.length} nutrientes omitidos (no disponibles)`;
      }
...
function guardarMezcla() {
  ...
  document.querySelectorAll('#tabla-nutrientes tr').forEach(row => {
    ...
    if (select && select.value) {
      nutrientesSeleccionados.push(parseInt(select.value));
    }
  });
  ...
  .then(data => {
    if (data.success) {
      alert(data.mensaje);
      window.location.href = '/mezclas';
    } else {
      alert(data.mensaje || 'Error al guardar la mezcla.');
    }
  })
  .catch(error => {
    console.error('Error:', error);
...
function confirmarGuardarComo() {
  ...
  // Verificar si se seleccionó una mezcla existente para actualizar
  const nombreExistente = document.getElementById('nombre-existente').value.trim();
  const endpoint = nombreExistente ? '/actualizar_mezcla' : '/guardar_mezcla_como';
  ...
  .then(response => response.json())
  .then(data => {
    alert(data.mensaje || 'Mezcla guardada.');
    if (data.success) window.location.href = '/mezclas';
  })
  .catch(error => {
    console.error('Error:', error);
...
function imprimirTabla() {
  ...
  // Nutrientes dinámicos: sólo mostrar la tabla si hay nutrientes vinculados a la mezcla
  const filasNutrientes = Array.from(document.querySelectorAll('#tabla-nutrientes tr')).filter(fila => {
    const select = fila.querySelector('select');
    return select && select.value;
  });

  if (filasNutrientes.length > 0) {
    contenido += `
      <h4>Aportes Nutricionales</h4>
      <table border="1" cellspacing="0" cellpadding="5" width="100%">
        <thead>
          <tr>
            <th>Nutriente</th>
        ...
        <tbody>
    `;
    filasNutrientes.forEach((fila, index) => {
      const select = fila.querySelector('select');
      const nombre = select?.selectedOptions[0]?.textContent?.trim() || '';
      const unidad = fila.querySelector(`input[name="unidad_${index}"]`)?.value || '';
      const min = fila.querySelector(`input[name="min_${index}"]`)?.value || '';
      const max = fila.querySelector(`input[name="max_${index}"]`)?.value || '';
      const valor = fila.querySelector(`#resultado-bs-${index}`)?.textContent?.trim() || '';
      if (nombre && valor !== '') {
        contenido += `<tr><td>${nombre}</td><td>${valor}</td><td>${min}</td><td>${max}</td><td>${unidad}</td></tr>`;
      }
    });
    contenido += `
        </tbody>
...
function agregarFilaDesdeDatos(ing, index) {
  ...
  const pesoBachada = ((inclusion * tamanoBachada) / 100).toFixed(2);
  ...
  const valorTotal = (pesoBachada * precioIngrediente).toFixed(2);
  ...
  fila.innerHTML = `
      ...
      <td><input type="number" class="form-control form-control-sm" name="peso_bachada_${index}" value="${pesoBachada}" readonly></td>
      <td><input type="number" class="form-control form-control-sm" name="costo_ingrediente_${index}" value="${precioIngrediente}" readonly></td>
      <td><input type="number" class="form-control form-control-sm" name="valor_${index}" value="${valorTotal}" readonly></td>
      <td>
...
function precargarIngredientes() {
  if (typeof window.ingredientesPrecargados !== 'undefined' && Array.isArray(window.ingredientesPrecargados) && window.ingredientesPrecargados.length > 0) {
    ...
    if (typeof window.mineralesTemplate !== 'undefined') {
      window.mineralesTemplate.forEach(m => {
        const nutrientesData = JSON.stringify(m.nutrientes).replace(/"/g, '"');
...
// Prevención de cierre o cambio de página con ingredientes cargados
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Prevenir navegación con ingredientes cargados
  document.querySelectorAll('a.nav-link').forEach(link => {
    link.addEventListener('click', function (e) {
      ...
      if (tieneIngredientes) {
        const confirmacion = confirm('Tienes ingredientes cargados. ¿Estás seguro que deseas salir y perder los cambios?');
        if (!confirmacion) {
          e.preventDefault();
        }
      }
  ...
  // Mostrar valores de min/max como enteros y guardar como centesimal
  document.querySelectorAll('input[name^="min_"], input[name^="max_"]').forEach(input => {
    if (input.value) {
