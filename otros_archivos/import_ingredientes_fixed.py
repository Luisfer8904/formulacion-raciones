...
def import_ingredientes():
    ...
    for index, row in df.iterrows():
        try:
            ...
            energia = str(row['EMA_AVES_Kcal_kg']) if pd.notna(row['EMA_AVES_Kcal_kg']) else None
            ...
            cursor.execute("""
                INSERT INTO ingredientes (
                    INGREDIENTES, PRECIO, COMENTARIOS, EMA_AVES_Kcal_kg, PB_, FB_,
                    Ca_, P_, LYS_, MET_
                ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """, (
                ingrediente, precio, comentarios, energia, proteina, fibra,
