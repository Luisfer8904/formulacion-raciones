...
def ver_nutrientes():
    ...
    cursor.execute("SELECT id, nombre, unidad, tipo FROM nutrientes WHERE usuario_id = %s ORDER BY nombre ASC", (session['user_id'],))
...
def guardar_nutriente():
    # Agregar tipos "Aditivos" y "Otros"
    tipos_nutrientes = ['Energético', 'Proteico', 'Mineral', 'Aditivo', 'Otros']
    ...
    if not nombre or not unidad or not tipo or tipo not in tipos_nutrientes:
        flash('Todos los campos son obligatorios y el tipo debe ser válido.', 'danger')
